...
from mothertongue.models import TranslatedModelBase

# Create generic test model.
class TestModel(TranslatedModelBase):
...
class TestModelTranslation(models.Model):
    ...
    obj_lang = models.CharField(max_length=len(settings.LANGUAGES)-1, choices=settings.LANGUAGES[1:])
    test_field_1 = models.CharField(_('test_field_1'), max_length=200)
    test_field_2 = models.TextField(_('test_field_2'), blank=True)
    
    class Meta(object):
        unique_together = (('test_model_instance', 'obj_lang'),)
    
    def __unicode__(self):
        return u'%s' % self.obj_lang